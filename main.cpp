...
void createAccount(Account acc[], int &count) {
    ...
    while (true) {
        ...
        if (!valid) { cout << "Letters and numbers only.\n"; continue; }
        if (usernameExists(acc, count, u)) { cout << "Username exists.\n"; continue; }
        break;
    }
    ...
    if (!f) { cout << "Error saving account!\n"; return; }
    f << u << " " << p << "\n";
    f.close();
...
int main() {
    ...
    while (true) {
        ...
        if (c == 1) {
            ...
            else { acc[idx].role == "admin" ? sys.adminMenu() : sys.userMenu(u); }
        }
        else if (c == 2) { createAccount(acc, accCount); }
    }
    return 0;
}
