...
void Movie::display() {
    cout << id << " | " << title << " | " << genre << " | " << duration << " mins | RM " << price << endl;
}
...
void Ticket::display() {
    cout << "Ticket " << ticketId << " | Movie: " << movieId << " | User: " << username << " | Seat: " << seatNumber << " | Total: RM " << total << endl;
}
...
void CinemaSystem::loadMovies() {
    ...
    if (!f) {
        cout << "movies.txt not found.\n";
        return;
    }

    while (f >> movies[movieCount].id >> movies[movieCount].title >> movies[movieCount].genre >> movies[movieCount].duration >> movies[movieCount].price) {
        for (int i = 0; i < 10; i++) {
...
void CinemaSystem::sortMoviesByTitleAsc() {
    for (int i = 0; i < movieCount - 1; i++) {
        ...
        for (int j = i + 1; j < movieCount; j++) {
            if (movies[j].title < movies[mn].title) {
                mn = j;
            }
        }
...
void CinemaSystem::cancelTicket(const string &user) {
    ...
    for (int i = 0; i < ticketCount; i++) {
        if (tickets[i].ticketId == tid && tickets[i].username == user) {
            pos = i;
        }
    }
...
void CinemaSystem::runSortMenu() {
    ...
    if (c == 1) {
        for (int i = 0; i < movieCount - 1; i++) {
            ...
            for (int j = i + 1; j < movieCount; j++) {
                if ((asc && movies[j].title < movies[mn].title) || (!asc && movies[j].title > movies[mn].title)) {
                    mn = j;
                }
            }
    ...
    if (c == 2) {
        for (int i = 0; i < movieCount - 1; i++) {
            for (int j = 0; j < movieCount - i - 1; j++) {
                if ((asc && movies[j].price > movies[j + 1].price) || (!asc && movies[j].price < movies[j + 1].price)) {
    ...
    if (c == 3) {
        for (int i = 1; i < movieCount; i++) {
            ...
            while (j >= 0 && ((asc && movies[j].duration > key.duration) || (!asc && movies[j].duration < key.duration))) {
                movies[j + 1] = movies[j];
                j--;
            }
